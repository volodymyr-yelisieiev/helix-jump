...
export default function Platform({
                                     ...
                                     poleRadius = 5,
                                     platformWidth = 1,
                                     sectorsList = [[0, Math.PI / 8, 'loss'], [Math.PI / 8, 3 * Math.PI / 8, 'hole'], [3 * Math.PI / 8, Math.PI / 2, 'loss'], [Math.PI / 2, Math.PI, 'platform'], [Math.PI, 9 * Math.PI / 8, 'loss'], [9 * Math.PI / 8, 11 * Math.PI / 8, 'hole'], [11 * Math.PI / 8, 3 * Math.PI / 2, 'loss'], [3 * Math.PI / 2, 2 * Math.PI, 'platform']],
    ...
    const {openModal, preScore, setPreScore, score, setScore, numJumps, setNumJumps} = useAppContext();

    const sectors = sectorsList.map(([startAngle, endAngle, type], i) => {
        const color = type === 'loss' ? lossColor : (type === 'platform' ? platformColor : 'transparent');
        ...
        return (<RigidBody
            ...
            onIntersectionEnter={(other) => {
                if (index >= preScore && other.rigidBodyObject?.name === 'ball') {
                    if (type === 'platform') {
                        other.rigidBody?.setLinvel({x: 0, y: 20, z: 0}, true);
                        if (index !== 0) {
